...
#endif
{
    ...
    cts_data->charger_detect_workqueue = create_singlethread_workqueue(CFG_CTS_DEVICE_NAME "-charger_detect_workqueue");
    if (cts_data->charger_detect_workqueue == NULL) {
        cts_err("Create charger detect workqueue failed");
        ret = -ENOMEM;
        goto err_destroy_charger_workqueue;
    }
#endif
...
#ifdef CONFIG_CTS_CHARGER_DETECT	
	cts_init_charger_detect(cts_data);
#endif
...
#ifdef CONFIG_CTS_CHARGER_DETECT
	destroy_workqueue(cts_data->charger_detect_workqueue);
err_destroy_charger_workqueue:
#endif
...
static const struct of_device_id cts_i2c_of_match_table[] = {
    ...
    { .compatible = "mediatek,tpd-chipone-tddi", },
    { .compatible = "mediatek,tpd-chipone-txd", },
};
